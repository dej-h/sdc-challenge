...
def findTarget(llines, rlines, horizonh, img, wl = 1, wr = 1, weight = 1, bias = 0, draw = 1): #returns False if non found, otherwise returns target x
    ...
    if not llines and not rlines:#rlines is not None and llines is not None:
        ...
        #linemaskL = getLineRoiMask(lline, margin=5)
        #linemaskR = getLineRoiMask(rline, margin=5)
        #linemask = cv2.bitwise_or(linemaskR, linemaskL)
            ...
            while (True):
                ...
                if lines is not None:
                    ...
                    if not llines and not rlines:#rlines is not None and llines is not None:
                        ...
                        wl = 1
                        wr = 1
                    target = findTarget(llines, rlines, hy, frame, wl, wr, draw = 0)#,bias = -212, draw = 1)
